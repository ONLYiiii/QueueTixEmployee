        ...
        fetch(getURL() + "qrcode/verify/", {
            ...
            if (response.status === 200) {
                ...
                if (result.status === "no ticket") {
                    ...
                    setFetchData(result);
                    setmassageFail("ตั๋วนี้ยังไม่สามารถใช้ได้วันนี้");
                    setShowModal(true);
            ...
            >
                ...
                >
                    ...
                    <Text style={{ fontSize: 16, textAlign: "center" }}>
                        {success
                            ? `ใช้${fetchData.type} - บัตร${
                                  fetchData.priceType === "Adult" ? "ผู้ใหญ่" : "เด็ก"
                              } สำเร็จ`
                            : `ใช้บัตรไม่สำเร็จ`}
                    </Text>
                    ...
                    {messageFail === "ตั๋วนี้เคยใช้ไปเเล้ว" ? (
                        <TouchableOpacity
                            onPress={() => {
                                setShowModal(false);
                                navigation.navigate("Ticket", { _id: fetchData._id });
                            }}
                        >
                        ...
                        <Text style={{ fontSize: 14 }}>
                            {success ? `Time Check in : ${getFullTime(new Date(fetchData.updated_at!))}` : messageFail}
                        </Text>
                    )}
                    <TouchableHighlight
                        onPress={() => {
